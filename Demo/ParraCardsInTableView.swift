...
class ParraCardsInTableView: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(
            ParraCardTableViewCell.self,
            forCellReuseIdentifier: ParraCardTableViewCell
                .defaultCellId
        )

        ParraFeedback.shared
            .fetchFeedbackCards(
                appArea: .id("07eb0d9a-4912-46b8-b77e-3741753960ac")
            ) { [self] cards, error in
                if let error {
                    ...
                    navigationItem
                        .prompt = "No Parra cards currently available."
        ...
        -> UITableViewCell
    {
        if let parraCell = tableView.dequeueReusableCell(
            ...
            for: indexPath
        ) as? ParraCardTableViewCell, indexPath.row == Constant.feedbackRow,
            shouldShowFeedback
        {
