...
//
// ! IMPORTANT: Go read this before deciding to delve into custom crash
//              reporting or signal handling.
//
...
class MetricManager: NSObject, MXMetricManagerSubscriber {
    ...
    private var crashes = Set<CrashInfo>()
        ...
        for diagnostic in diagnostics {
            for crash in diagnostic.crashDiagnostics ?? [] {
                crashes.insert(
                    CrashInfo(
                        crash: crash,
                        endDate: diagnostic.timeStampEnd
                    )
                )
            }
        }
